...
void TicTacToeGameController::saveGameSlot(std::string filename){
	...
	QFuture<void> f = run(boost::bind(&GameStrategy::saveGame, gameStrategy_, filename));
...
/* There are two ways of starting a new game set:
FIRST: to generate an oponent player : */
...
/*SECOND: to load an oponent player : */
...
(TicTacToePlayer::PlayerSign humanPlayerSign, std::string computerPlayerFilename){
	...
	//here start a new thread
	QFuture<void> f = run(boost::bind(&GameStrategy::loadGame, gameStrategy_, computerPlayerFilename));
	serWatcher_.setFuture(f);
